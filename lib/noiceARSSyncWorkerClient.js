    ...
    return(new Promise((toot, boot) => {
        ...
        if (
            ...
            that.DBs[that.threadInfo.formDBMappings[args.schema].dbTagName].get({
                ...
                that.dbOutputFilter(args.schema, row, (args.hasOwnProperty('bypassFilters') && (args.bypassFilters === true))).then((row) => {
                    let out = {};
            ...
            dbi.getAll(funcArgs).then((bros) => {
                ...
                Promise.all(bros.map((row) =>{
                    return(new Promise((_t, _b) => {
                        that.dbOutputFilter(args.schema, row, (args.hasOwnProperty('bypassFilters') && (args.bypassFilters === true))).then((fRow) => {
                            rows.push(fRow);
                            _t(true);
                        }).catch((error) => {
                        ...
                        Object.keys(cData).filter((colName) => {
                            ...
                            switch(that.threadInfo.formDefinitions[schema].idIndex[fieldID].datatype){
                                case "TIME":
                                    if (/^\d+$/.test(cData[colName])){
                                        try {
                                            let t = that.fromEpoch(cData[colName], 'dateTimeLocale');
                                            if (that.isNotNull(t)){
                                                cData[colName] = t;
                                            }else{
                                                throw("invalid date format");
