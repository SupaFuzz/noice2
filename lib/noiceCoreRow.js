...
set rowData(v){
    if (v instanceof Object){
        ...
        Object.keys(v).forEach((colName) => { this.getDataElement(colName, v[colName]) }, this);

        // prune dataElements that no longer exist in the input
        Object.keys(this.dataElements).filter((fieldID) => {return(!(v.hasOwnProperty(fieldID)))}, this).forEach((fieldID) => {
    ...
    return(new Promise((toot, boot) => {
        if (that.isNotNull(colName)){
            ...
            if (! (that.dataElements.hasOwnProperty(colName))){
                ...
                new Promise((_t) => {_t(((that.isNotNull(defaultValue) && that.dataElements.hasOwnProperty(colName)))?that.dataElements.setValue(defaultValue, true):false)}).then(() => {
                    toot(that.fieldConfig[colName]);
                }).catch((error) => {
