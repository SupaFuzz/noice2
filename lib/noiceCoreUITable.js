...
renderRows(){
    ...
    let rows = [];
    that.rows.forEach((row, idx) => {
        if (row instanceof Object){
            rows.push(`<div class="${that.dataRowClassName}" data-selected="false" data-osortorder="${idx}">${that.columns.sort((a,b) => {return(a.order - b.order)}).map((col) => {return(
                `<span class="${that.dataRowColClassName}" data-name="${col.name}" data-sort="none">${((row instanceof Object) && row.hasOwnProperty(col.name))?row[col.name]:'&nbsp'}</span>`
            )}).join("")}</div>`);
        }
    });
    that._DOMElements.tableListContainer.insertAdjacentHTML('afterbegin', rows.join(""));
    ...
    // setup the select handler thing
    Array.from(that._DOMElements.tableListContainer.querySelectorAll(`div.${that.dataRowClassName}`)).forEach((el, idx) => {
        el.dataset.rownum = idx;
        el.addEventListener('click', (evt) => { that.handleRowSelect(el); });
        that.applyRowCSS(el);
    });
}
        ...
        if (hdrColEl.dataset.sort == "none"){
            if (Array.from(that._DOMElements.tableHeader.querySelectorAll(`span.${that.headerColClassName}:not([data-sort="none"])`)).length == 0){
                Array.from(that._DOMElements.tableListContainer.querySelectorAll(`div.${that.dataRowClassName}`)).sort((a,b) => {
                    return(parseInt(a.dataset.osortorder) - parseInt(b.dataset.osortorder))
                }).forEach((el) => {
