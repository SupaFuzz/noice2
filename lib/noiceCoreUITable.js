/*
    ...
    * handleCellEdit(rowElement, cellElement)
    ...
    whatever ya want:
        ...
        .noiceCoreUITable .tablePrefEditor {
        ...
        if (
            ...
            that.editCellCallback(rowElement, cellElement, that).then((value) => {
                ...
                new Promise((_t,_b) => {
                    if ((that.selectMode == 'multiple') && (that.numSelectedRows > 1)){
                        if (that.modifyAll == "prompt"){
                            ...
                            that.userQuery({
                                ...
                                detail: `this value of '${cellElement.dataset.name}' will by copied to all other selected rows. This will modify ${that.numSelectedRows} rows`,
                    ...
                    /*
                        LOH 2/12/24 @ 2305
                        it throws up the prompt, it just doesn't actually do the modifyAll yet
                        we need to abstract the memed section below into a function what returns
                        on a them there promises so we can put it in a Promise.all and do the
                        whole mess of em.

                        keeping in mind if it's a lot, and the callbacks will be hitting indexedDB
                        it could get slow. We may very well need to animationFrame sync this as well

                        and also the nesting in here is deeper than the mind of minolta
