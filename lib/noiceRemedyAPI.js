...
parseXHR(){
    ...
    if (self.hasAttribute('xhr')){
        ...
        if (
            (self.isNull(self.xhr.responseType) || (self.xhr.responseType == 'text')) &&
            (self.isNotNull(self.xhr.responseText) || self.isNotNull(self.xhr.response) )
        ){
            try {
    ...
    */
    ...
    if (p.fetchAttachments){
        ...
        Object.keys(data.values).forEach(function(field){
            if (self.isNotNull(data.values[field])){
    ...
    if (p.hasOwnProperty('fetchMenus') && (p.fetchMenus == true)){
        ...
        try {
            ...
            tmp.forEach(function(fieldDef){
                if (
                    ...
                    (self.isNotNull(fieldDef.limit.char_menu))
                ){
                    // push a getMenu here
        ...
        Object.keys(menusToGet).forEach(function(menuName){
            pk.push(new Promise(function(toot, boot){
                let abrt = false;
                self.getMenu({name: menuName}).catch(function(error){
                    abrt = true;
                    menuErrors.push(error);
                    if (self.debug){ console.log(`${this._className} | getFormFields()/fetchMenus(${menuName}) | error: ${error}`); }
                    boot(false);
