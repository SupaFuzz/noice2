...
getFormView(defaults){
    if (! (this.formView instanceof Element)){
        this.formView = new wcARSFormView(Object.assign({}, {
            debug: this.debug,
...
/*
    setFieldValueFromView(fieldName, fieldValue, formElement, selfRef)
...
setFieldValueFromView(fieldName, fieldValue, formElement, selfRef){
    ...
    if (this.dataElements.hasOwnProperty(fieldName) && (this.dataElements[fieldName].value != fieldValue)){

        // log it if we're in debug
        this.log(`[${this.formName}]: ${this.entryId} | setFieldValueFromView() | [${fieldName}]: ${fieldValue}`, true);

        // this seems rather lame but whatever
        let tmp = {};
        tmp[fieldName] = fieldValue;

        // technically makes this an async function (though nothing awaits it, could be handy)
        return(this.modify(tmp));
    }
}
...
valueChange(args){
    let that = this;
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`[${that.formName}]: ${that.entryId} | valueChange() | ${args.fieldID} | [new]: ${args.newValue} | [old]: ${args.oldValue}`); }
        ...
        this.updateViews(args.fieldID, args.newValue); // note might need to change the value ref
...
updateViews(fieldID, value){
    ...
    /*
        LOH 4/30/24 @ 1632
        the below works but note we can't check this:

            (this.formView.formElements[fieldID] instanceof wcFormElement)

        because we haven't loaded wcFormElement loaded in here and we shouldn't
        this code to update a formElement (and hence the type validation for it)
        belong in something like wcARSFormView.updateField(...) or something like that

        so there's a thread to pull on in the morning.

        here's another.
        we need type coersion on this.dataElements -- they're born in here knowing
        the ARS type so i think we're good on the required meta. But basically we need
        to handle that here -- for instance try this:

            arsRow.modify({'Purchase Order Line': 5})

        sets the change flag, updates the field and lights up the save button
        all good right? now set it back to the original value

            arsRow.modify({'Purchase Order Line': 3})

        changeFlag still set and the save button still lit up.

        beacause 'Purchase Order Line' is a char not an int
        and the "this is the same value"? comparison won't work.
        
    */

    // formView is all I've got for the moment. there will be more
    if (
        (this.formView instanceof wcARSFormView) &&
        (this.formView.formElements instanceof Object) &&
        (this.formView.formElements[fieldID] instanceof Object)
    ){
        this.formView.formElements[fieldID].value = value;
    }
}
