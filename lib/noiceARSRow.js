...
getRunningConfig(){
    ...
    Object.keys(this.dataElements).forEach((fieldName) => {
...
/*
    fieldDisplayStateChange({ fieldID: <colName>, display: <bool>, oldDisplay: <bool>})
    this fires when a dataElement's .display boolean has changed value
    here we distribute that change to any instantiated UIs
*/
fieldDisplayStateChange(args){
    // debug
    this.log(`[${this.formName}]: ${this.entryId} | fieldDisplayStateChange(${args.fieldID}, ${args.display})`, true);

    if ((this.formView instanceof wcARSFormView) && (args.display != args.oldDisplay)){
        this.formView.updateFieldProperties(args.fieldID, "display", args.display, args.oldDisplay);
    }
}
...
unsetDataElementAttribute(fieldName, attributeName){
    if ((this.dataElements[fieldName] instanceof Object) && (this.fieldConfig[fieldName] instanceof Object)){
        ...
        /* LOL NO -- the running config has the current value LOL
        let rc = this.getRunningConfig();
        if ((rc[fieldName] instanceof Object) && rc[fieldName].hasOwnProperty(attributeName)){
            this.dataElements[fieldName][attributeName] = rc[fieldName][attributeName];
        }
        */
    }
}
