...
setFieldValueFromView(fieldName, fieldValue, formElement, selfRef){
    ...
    console.log(`setFieldValueFromView() | [${fieldName}]: ${fieldValue}`);

    // yeah I dunno ... just do ieeet
    let tmp = {};
    tmp[fieldName] = fieldValue;

    this.modify(tmp).then((rowdata) => {
        console.log('row modify success?', rowdata)
    }).catch((error) => {
        console.log('row modify failure?', error);
    })
}
