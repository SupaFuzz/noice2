...
fieldValidationStateChange(args){
    ...
    /*
        LOH 5/1/24 @ 1645 -- COB
        yes, its on LOL. very very on.
        this is getting called 4 times per field, and after tracking it down that's actually correct
        for instance setting 'Purchase Order Line' null
        this fires
            1) to clear errors at the top of validate()
            2) to clear warnings at the top of validate();
            3) to set the nullable error
            4) to clear the enforceMenu warning when we do that check

        I'm not sure what the correct way to deal with this is

        on the one hand as long as we aren't logging the hell out of it here the multiple calls
        aren't an issue. getting called this often is the legit behavior it's just extremely
        verbose

        so when it comes to logging and changing states, we're just gonna have to check the
        field in question, be sure not to add the same thing twice and only log when somehting
        is actually changing.

        so that's where to pick up in the AM.
        let's get the validation errors on screen
    */
