        ...
        that.threadClient.getARSRow({
            ...
            that.setRowData(row).then(() => {
                ...
                new Promise((_t) => {_t((that.dataLoadedCallback instanceof Function)?that.dataLoadedCallback(that.rowData, that):false)}).then(() => {
                    ...
                    that.log(`[${that.formName}]: load(${entryId}) | successfully updated dataElements | dataLoadedCallback() threw unexpectedly: ${error}`);
                    boot(error);
                });
            }).catch((error) => {
...
addDataElementCallback(dataElement){
    ...
    return(new Promise((toot, boot) => {
        ...
        /*
            fetch fields with a menuQuery
            and set dataElement.values
            note: this should be it's own function because we may need
            to call it in other contexts but for now just workin it out
        */
        if (
            (that.fieldConfig[dataElement.fieldName] instanceof Object) &&
            (that.fieldConfig[dataElement.fieldName].menuQuery instanceof Object) &&
            that.fieldConfig[dataElement.fieldName].menuQuery.hasOwnProperty('function') &&
            that.isNotNull(that.fieldConfig[dataElement.fieldName].menuQuery.function) &&
            (that.fieldConfig[dataElement.fieldName].menuQuery.args instanceof Object)
        ){
            that.threadClient[that.fieldConfig[dataElement.fieldName].menuQuery.function](that.fieldConfig[dataElement.fieldName].menuQuery.args).then((rows) => {
                dataElement.values = (that.fieldConfig[dataElement.fieldName].menuQuery.mapper instanceof Function)?that.fieldConfig[dataElement.fieldName].menuQuery.mapper(rows):rows;
                that.fieldConfig[dataElement.fieldName].values = dataElement.values;
                toot(true)
            }).catch((error) => {
