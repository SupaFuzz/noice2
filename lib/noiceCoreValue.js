...
setValue(v, bypassBool, bypassEditable){
    let that = this;
            ...
            if (bypassBool === true){
                ...
                new Promise((_t,_b) => { _t((that.valueChangeCallback instanceof Function)?that.valueChangeCallback(that.dataTypeMunger(v), that._previousValue, that):that.dataTypeMunger(v)); }).then((cVal) => {
                    ...
                    */
                    ...
                    new Promise((_t,_b) => { _t((that.valueChangedCallback instanceof Function)?that.valueChangedCallback(that._value, that._previousValue, that):that._value); }).catch((error) => {
...
*/
get display(){ return(this._display === true); }
set display(v){
    let oldVal = this._display;
    this._display = (v === true);
    if (this.displayStateChangeCallback instanceof Function){ this.displayStateChangeCallback(this.display, oldVal, this); }
}
