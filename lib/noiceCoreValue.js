    ...
    return(new Promise((toot, boot) => {
        if ((that.editable) || (bypassEditable === true)){
            ...
            if (bypassBool === true){
                ...
                new Promise((_t,_b) => { _t((that.valueChangeCallback instanceof Function)?that.valueChangeCallback(that.dataTypeMunger(v), that._undoValue, that):that.dataTypeMunger(v)); }).then((cVal) => {
...
set changeFlag(v){
    ...
    if ((! (v === true)) && (this._changeFlag === true) && (this.undoValue != this.value)){ this.undoValue = this.value; }
