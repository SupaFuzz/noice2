                    ...
                    Object.keys(that.formMappings).map((formName) => {return(new Promise((_t,_b) => {
                        ...
                        */
                        ...
                        api.getFormFields({ schema: formName }).then((formDef) => {
                            ...
                            if (
                                (mustGet.indexOf(formName) >= 0) &&
                                (that.staticFormDefinitions instanceof Object) &&
                                (that.staticFormDefinitions[formName] instanceof Object)
                            ){
                                // an absolutely last ditch fallback because getFormFields is hella flakey serverside (thx BMC)
                                that.log(`${fn}() | getFormFields(${formName}) threw unexpectedly (using staticFormDefinitions default): ${error}`);
                                ...
                                _bulkPutQueue.push({
                                    ...
                                    schema: formName,
                                    fetchDate: 0,
                                    value: that.staticFormDefinitions[formName]
                                });
                                complete ++;
                                _updateIt();
                                _t(true);
...
handleBulkSync(formMeta, api, entryIdExcludeList){
    ...
    return(new Promise((toot, boot) => {
        ...
        if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync instanceof Object){
            ...
            // check metaQuery
            that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'checking for new bulk sync file', runAnimation:true, functionName: fn });
            api.query(that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.metaQuery).then((metaQueryResult) => {
                if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.hasNewDataCallback(metaQueryResult, formMeta, that)){
                    ...
                    that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'downloading bulk sync file', runAnimation:false, functionName: fn });
                    ...
                    that.statusUpdate({ message: `sync: ${formMeta.formName}`, runAnimation:false, functionName: fn });
