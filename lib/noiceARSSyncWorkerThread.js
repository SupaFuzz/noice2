...
import { noiceRemedyAPI, noiceRemedyAPIException } from './noiceRemedyAPI.js';


class noiceARSSyncWorkerThread extends noiceWorkerThread {
...
constructor(args, defaults, callback){
    super(args, noiceObjectCore.mergeClassDefaults({
        _version: 1,
        _className: 'noiceARSSyncWorkerThread',
        threadName: 'noiceARSSyncWorkerThread',
        debug: false,
        _config: {},
        _hasConfig: false,
        threadLocks: {},
        ...
        staticStoreDefinitions: {},
        formDefinitions: {},
        apiTimeout: (60 * 30 * 1000), // api connects time out after 30 minutes,
    ...
    */
    Object.assign(this.staticStoreDefinitions, {
        /*
...
initializeThread(data, event){
    ...
    that.threadLocks[fn] = new Promise((toot, boot) => {
        ...
        let mergeOut = {
            dbConfig: {
                dbName: that.config.dbConfig.dbName,
                dbVersion: that.config.dbConfig.dbVersion,
                storeDefinitions: {}
            },
        ...
        if (that.initialized == true){
            ...
            // mount the indexedDB and all that entails
            that.mountDB().then((db) => {

                // merge just the minimum to mount the db from an external thread into the output
                Object.keys(db.storeDefinitions).forEach((storeName) => {
                    mergeOut.dbConfig.storeDefinitions[storeName] = {
                        createOptions: db.storeDefinitions[storeName].createOptions,
                        indexes: db.storeDefinitions[storeName].indexes
                    }
                });
                ...
                that.updateFormDefinitions().then((formDefinitions) => {
                    ...
                    // merge formdefs to output
                    mergeOut.formDefinitions = formDefinitions;
                ...
                boot(`mountDB() threw unexpectedly: ${error}`);
            });
        }
...
set config(v){
    ...
    if (v instanceof Object){ this._config = v; }
    ...
    */
    if (
        (this._config.dbConfig instanceof Object) &&
        (this._config.dbConfig.storeDefinitions instanceof Object)
    ){
        this.statusUpdate({functionName: 'config attribute setter', message: 'parsing config'});

        Object.keys(this._config.dbConfig.storeDefinitions).filter((storeName) => {return(
            (this._config.dbConfig.storeDefinitions[storeName] instanceof Object) &&
            (this._config.dbConfig.storeDefinitions[storeName].indexes instanceof Object)  &&
            (this._config.dbConfig.storeDefinitions[storeName]._sync instanceof Object) &&
            (this._config.dbConfig.storeDefinitions[storeName]._sync.query instanceof Object) &&
            this._config.dbConfig.storeDefinitions[storeName]._sync.query.hasOwnProperty('schema') &&
            this.isNotNull(this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema)
        )}, this).forEach((storeName) => {

            this.formMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema] = storeName;
            this.storeMappings[storeName] = this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema;

            Object.keys(this._config.dbConfig.storeDefinitions[storeName].indexes).filter((indexName) => {return(
                (this._config.dbConfig.storeDefinitions[storeName].indexes[indexName] instanceof Object) &&
                this._config.dbConfig.storeDefinitions[storeName].indexes[indexName].hasOwnProperty('_id') &&
                this.isNotNull(this._config.dbConfig.storeDefinitions[storeName].indexes[indexName]._id)
            )}, this).forEach((indexName) => {
                if (! (this.formIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema] instanceof Object )){ this.formIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema] = {}; }
                if (! (this.formIDToIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema] instanceof Object )){ this.formIDToIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema] = {}; }

                this.formIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema][this._config.dbConfig.storeDefinitions[storeName].indexes[indexName]._id] = indexName;
                this.formIDToIndexMappings[this._config.dbConfig.storeDefinitions[storeName]._sync.query.schema][indexName] = this._config.dbConfig.storeDefinitions[storeName].indexes[indexName]._id;
            }, this)
        }, this)
    }

    // LOOSE END: might wanna fire some stuff when the config changes
    // but honestly shouldn't change really. I dunno. for later. maybe
}
...
/*
    checkPendingUpgrade()
    resolve boolean true if the config specifies a dbVersion higher than the unmounted db
    else false (obvz)
*/
checkPendingUpgrade(){
    let that = this;
    ...
    return(new Promise((toot, boot) => {
        that.statusUpdate({ message: 'mounting database', detail: 'check for pending upgrade', functionName: fn });

        if (
            (that.config instanceof Object) &&
            (that.config.dbConfig instanceof Object) &&
            (that.config.dbConfig.hasOwnProperty('dbName')) &&
            (that.isNotNull(that.config.dbConfig.dbName)) &&
            (that.config.dbConfig.hasOwnProperty('dbVersion')) &&
            (! isNaN(parseInt(that.config.dbConfig.dbVersion)))
        ){
                ...
                if (that.threadHandle.indexedDB.databases instanceof Function){
                    that.threadHandle.indexedDB.databases().then((dbList) => {
                        let bs = dbList.filter((a)=>{return(
                            ...
                            (a.name == that.config.dbConfig.dbName)
                        )});
                        toot(
                            (bs.length > 0) &&
                            bs[0].hasOwnProperty('version') &&
                            (bs[0].version < parseInt(that.config.dbConfig.dbVersion))
                        );
                    }).catch((error) => {
            ...
            that.log(`${fn}() | config does not contain dbConfig.dbVersion or dbConfig.dbName?! defaulting false`);
            toot(false);
        }
    }))
}
...
mountDB(){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.statusUpdate({ message: 'mounting database', functionName: fn });
        that.checkPendingUpgrade().then((pendingUpgrade) => {

            new Promise((_t, _b) =>{
                if (pendingUpgrade && (
                    (that.config instanceof Object) &&
                    (that.config.dbConfig instanceof Object) &&
                    (that.config.dbConfig.preUpgradeExec instanceof Function)
                )){
                    that.config.dbConfig.preUpgradeExec(that).then((preUpgradeExecOutput) => { _t(preUpgradeExecOutput); }).catch((error) => { _b(error); })
                }else{
                    _t(false);
                }
            }).catch((error) => {
                that.log(`${fn}() | preUpgradeExec() threw unexpectedly, naively continuing to upgrade: ${error}`);
                ...
                */
                new noiceIndexedDB({
                    dbName: that.config.dbConfig.dbName,
                    dbVersion: that.config.dbConfig.dbVersion,
                    storeDefinitions: Object.assign({}, that.config.dbConfig.storeDefinitions, that.staticStoreDefinitions)
                }).open({
                    destructiveSetup: true,
                    setupCallback: (dbInstance) => {
                        that.statusUpdate({
                            message: 'mounting database',
                    ...
                    that.dbStats = {};
                    new Promise((_t,_b) => {
                        that.db.getDescription().then((desc) => {
                            desc.forEach(function(tbl){
                                if (that.debug){ that.log(`${fn}() | ${tbl.name}: ${tbl.count}`); }
                                that.dbStats[tbl.name] = tbl;
                            });
                            _t(true);
                        }).catch((error) => {
                            // just noting it because we mounted at least so let's keep going
                            that.log(`${fn}() | database successfully mounted | getDescription() threw unexpectedly (ignored): ${error}`);
                            _t(true);
                        })
                    }).then(() => {
                        that.statusUpdate({
            ...
            that.log(`${fn}() |checkPendingUpgrade() threw unexpectedly (this should not happen): ${error}`);
            boot(error);
        });
    }));
}
...
updateFormDefinitions(){
    ...
    return(new Promise((toot, boot) => {
        that.statusUpdate({ message: 'loading form definitions', functionName: fn });
        that.db.getAll({
            storeName: 'meta',
            indexName: 'type',
            ...
            */
            ...
            that.getAPI().then((api) => {
                ...
                let complete = 0;
                let _bulkPutQueue = [];
                function _updateIt(formName){
                    that.statusUpdate({
                        message: 'loading form definitions',
                        detail: `${complete} of ${Object.keys(that.formMappings).length}`,
                        additionalDetail: formName,
                ...
                Promise.all(
                    Object.keys(that.formMappings).map((formName) => {return(new Promise((_t,_b) => {

                        /*
                            NOTE 8/11/23 @ 1248
                            the REST API will straight up die with an http/500
                            on any getFormFields call against a form that was
                            created via the arstudio "save as copy" feature

                            no. shit.

                            Don't know if that's like ... fixed in a newer version?
                            20.02 is like 4 years out of date so maybe? In any case
                            verified that this morning beyond any shadow of a doubt

                            anyhoo. it is what it is.
                        */

                        api.getFormFields({ schema: formName }).then((formDef) => {
                            
                            // handle formDefinitionTransform if we have one
                            if (
                                (that.formMappings instanceof Object) &&
                                that.formMappings.hasOwnProperty(formName) &&
                                (that.config.dbConfig.storeDefinitions[that.formMappings[formName]] instanceof Object) &&
                                (that.config.dbConfig.storeDefinitions[that.formMappings[formName]]._sync instanceof Object) &&
                                (that.config.dbConfig.storeDefinitions[that.formMappings[formName]]._sync.formDefinitionTransform instanceof Function)
                            ){
                                formDef = that.config.dbConfig.storeDefinitions[that.formMappings[formName]]._sync.formDefinitionTransform(formDef);
                            }

                            _bulkPutQueue.push({
                                key: `formDefinition_${that.formMappings[formName]}`,
                                type: 'formDefinition',
                                schema: formName,
                            ...
                            that.log(`${fn}() | getFormFields(${formName}) threw unexpectedly: ${error}`);
                            _b(error);
                        });
                    }))})
                ).then(() => {

                    // bulk put what we got
                    that.statusUpdate({
                        message: 'caching fetched form definitions',
                        detail: '',
                        additionalDetail: '',
                        updatePieCharts: [{name: 'network', value: 0 }],
                        runAnimation: true,
                    ...
                    that.db.bulkPut({
                        storeName: 'meta',
                        objects: _bulkPutQueue
                    }).catch((error) => {

                        // this seems like a big deal, but not really because while the cache write failed, we did get everything we needed to keep truckin'
                        that.log(`${fn}() | ignored | failed to write ${_bulkPutQueue.length} updated formDefinitions to meta table: ${error}`);

                    }).then(() => {

                        // merge any new stuff we got
                        _bulkPutQueue.forEach((formDef) => {that.formDefinitions[formDef.schema] = formDef.value; });
                    ...
                    // same logic as getAPI() failure, it's all good as long as we aren't missing any
                    if (mustGet.length == 0){
                        that.log(`${fn}() | at least one failure occurred fetching form definitions (see above log) | using cached form definitions`);
                        toot(that.formDefinitions);
                    }else{
                        that.log(`${fn}() | at least one failure occurred fetching form definitions and missing required forms: ${mustGet.join(', ')} | cannot proceed. fatal error | ${error}`);
                        that.statusUpdate({
                            messageNumber: 3,
                            error: true,
                            message: 'failed to fetch missing form definitions, cannot proceed (please contact administrator)',
                            detail: `${error}`,
                            runAnimation: false,
                            functionName: fn
                        });
...
getAPI(){
    ...
    return (new Promise((toot, boot) => {
        if (navigator.onLine){
            if (
                (that.api instanceof noiceRemedyAPI) &&
                that.api.isAuthenticated &&
                (that.apiTimeout < (that.epochTimestamp() - that.api._authDate))
            ){
                toot(that.api);
            }else{
                ...
                new Promise((_t,_b) => {
                    if ((that.api instanceof noiceRemedyAPI) && that.api.isAuthenticated){
                        that.api.logout().then(() => {_t(true); });
                    }else{
                        _t(true);
                    }
                }).then(() => {
                    ...
                    new noiceRemedyAPI({
                        ...
                        user:       that.config.apiConnect.user,
                        password:   that.config.apiConnect.password,
    ...
    return (new Promise((toot, boot) => {
        that.awaitThreadLocks(['initializeThread']).catch((error) => {
            ...
            that.threadLocks[fn] = new Promise((_t, _b) => {
                ...
                that.syncAllForms().then(() => { _t(true); }).catch((error) => { _b(error); });

                /*
                    LOH 8/11/23 @ 1500 -- done for the week
                    everything works up to here, including the mainLoop.

                    next up: syncForm(), and all the things we need for that
                    then syncAllForms() to do 'em all

                    keep in mind where calls can come from and what should block them.
                    there will be times it's necessary to call syncForm() directly and the mainLoop
                    should wait on that. And we should test that thoroughly, because blocked threads
                    suuuuhuuuuck to debug
                ...
                */
                ...
                // be kind, rewind.
                setTimeout(() =>{ delete(that.threadLocks[fn]); }, 2);
            });
...
syncAllForms(){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.getAPI().then((api) => {
            ...
            Promise.all(
                Object.keys(that.formMappings).filter((a)=>{return(
                    (that.config.dbConfig.storeDefinitions[that.formMappings[a]] instanceof Object) &&
                    (that.config.dbConfig.storeDefinitions[that.formMappings[a]]._sync instanceof Object) && ( ! (
                        that.config.dbConfig.storeDefinitions[that.formMappings[a]]._sync.hasOwnProperty('enableSync') &&
                        (that.config.dbConfig.storeDefinitions[that.formMappings[a]]._sync.enableSync == false)
                    ))
                )}).sort((a,b) => {return(
                    that.config.dbConfig.storeDefinitions[that.formMappings[a]]._sync.syncOrder -
                    that.config.dbConfig.storeDefinitions[that.formMappings[b]]._sync.syncOrder
                )}).map((schema) => { return(that.syncForm(schema, api)); })
            ).catch((error) => {

                // one monkeywrench don't stop no show ... or ... something ... LOL
                that.log(`${fn}() | at least one form failed sync (see log) | ignored | ${error}`);
...
/*
    syncForm(schema, api, forceSync, forcePrune)
...
syncForm(schema, api, forceSync, forcePrune){
    ...
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`${fn}(${schema}) | start`); }
        that.statusUpdate({ message: `sync: ${schema}`, detail: 'loading meta data', functionName: fn });
            ...
            new Promise((_t,_b) => {
                ...
                that.handleTransmits(formMeta, api).then((entryIdIndex) => {
                    ...
                    */
                    ...
                    if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.rowUpdateCallback instanceof Function){
                        that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.rowUpdateCallback(that, entryIdIndex);
                    }


                    // do the sync if we're supposed to do
                    if ((forceSync == true) || ((that.epochTimestamp() - formMeta.lastSync) >= that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.hasOwnProperty('syncInterval')?that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.syncInterval:0)){

                        // handle bulkSync
                        let bulkSyncBypassed = false;
                        that.handleBulkSync(formMeta, api, handleTransmitExcludeList).catch((error) => {
                            bulkSyncBypassed = true;
                            if (error == "no new bulkSync data"){
                ...
                let pruneInterval = that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.hasOwnProperty('pruneInterval')?that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.pruneInterval:0;

                new Promise((___t, ___b) => {

                    // do the prune if we're supposed to (the sync subprocess should have resolved to an entryIdIndex)
                    if (
                        ((forcePrune == true) || (timeSinceLastPrune >= pruneInterval)) &&
                        (! (
                            that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.hasOwnProperty('disablePrune') &&
                            (that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.disablePrune == true)
                        ))
                    ){

                        that.handlePrune(formMeta, syncOutput, api).then(() => {
                            // log some stuff and be out
                            that.statusUpdate({ message: `sync: ${schema}`, detail: 'sync complete', functionName: fn });
                            ___t(true);
                        }).catch((error) => {
                    ...
                    that.handleWiteQueuePrune(formMeta, api).catch((error) => {
                         that.log(`${fn}(${schema}) | handleWiteQueuePrune() threw unexpectedly (ignored) error was: ${error}`);
                    }).then(() => {
                        if (that.debug){ that.log(`${fn}(${schema}) | complete`); }
                        toot(true);
                    })
...
handleBulkSync(formMeta, api, entryIdExcludeList){
    ...
    return(new Promise((toot, boot) => {
        ...
        if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync instanceof Object){

            // check metaQuery
            that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'checking for new bulk sync file', functionName: fn });
            api.query(that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.metaQuery).then((metaQueryResult) => {
                if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.hasNewDataCallback(metaQueryResult, formMeta, that)){

                    that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'downloading bulk sync file', functionName: fn });
                    api.getTicket(that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.dataFileQueryCallback(metaQueryResult, formMeta, that)).then((result) => {

                        // parse it and encode the rows for db insertion
                        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'parsing data ...', functionName: fn });
                        that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.parseDataFile(result, formMeta, that).then((parsed) => {

                            // filter entryIdExcludeList from parsed
                            if (entryIdExcludeList instanceof Array)
                            parsed = parsed.filter((row) => {
                                let entryId = row[that.formIndexMappings[formMeta.formName].hasOwnProperty('1')?that.formIndexMappings[formMeta.formName]['1']:'1'];
                                return(entryIdExcludeList.indexOf(entryId) < 0);
                            });
                            ...
                            let chunks = [];
                            let entryIdIndex = {};
                            while (parsed.writeQueue.length > 0){ chunks.push(parsed.writeQueue.splice(0, that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.bulkPutLimit)); }
                            Promise.all(chunks.map((chunk, idx) => {return(new Promise((_t,_b) => {
                                chunk.forEach((row) => {entryIdIndex[row.entryId] = true; });
                                that.db.bulkPut({
                                ...
                                */
                                that.writeFormMeta(formMeta, {
                                    lastBulkSyncFileDate: that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.getDataFileBuildDate(metaQueryResult, that)
                                }).then(() =>{
                                    that.statusUpdate({
                                        message: `sync: ${formMeta.formName}`,
...
handleDeltaSync(formMeta, api, maxModifiedDate){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.getMaxModifiedDate(formMeta, maxModifiedDate).then((mmd) => {
            ...
            */
            let q = Object.assign({}, that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.query);
            ...
            api.query(q).then((result) =>{
                ...
                while (result.entries.length > 0){
                    chunks.push(result.entries.splice(0, that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.bulkPutLimit).map((row) => {
                ...
                Promise.all(chunks.map((chunk, idx) => {return(new Promise((_t,_b) => {
                    ...
                    /*
                        write meta stats
                    */
                    that.writeFormMeta(formMeta, { lastSync: that.epochTimestamp() }).then(() =>{
                        that.statusUpdate({
                            message: `sync: ${formMeta.formName}`,
...
getMaxModifiedDate(formMeta, maxModifiedDate){
    ...
    return(new Promise((toot, boot) => {
        ...
        if (that.isNotNull(maxModifiedDate)){
            ...
            let mmd = 0;
            let mmdField = that.formIDToIndexMappings[formMeta.formName].hasOwnProperty('6')?that.formIDToIndexMappings[formMeta.formName]['6']:'6';
            that.db.openCursor({
                storeName: formMeta.storeName,
...
ARSDataFilter(formMeta, inputFields){
    ...
    Object.keys(fields).filter((fieldID) => {return(
        ...
        switch(formMeta.formDef.idIndex[fieldID].datatype){
            case 'TIME':
    ...
    */
    Object.keys(fields).filter((fieldID) => { return(
        ...
        if ((! isNaN(parseFloat(fields[fieldID])))){
            fields[fieldID] = {
...
dataStoreFilter(formMeta, apiRow){
    ...
    Object.keys(apiRow).forEach((fieldName) => {
        ...
        if (
            ...
            fieldValue = that.toEpoch(fieldValue)
        }
...
coalesceWriteQueue(formMeta, writeQueueRaw){
    ...
    */
    if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.writeTransactionTransform instanceof Function){
        out = out.map((trans) => {return(
            that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.writeTransactionTransform(formMeta, trans, that)
        )});
    }
    ...
    return(new Promise((toot, boot) => {
        ...
        if (['createTicket', 'modifyTicket'].indexOf(trans.call) < 0){
            ...
            api[trans.call](trans.args).then((response) => {
                that.log(`${fn}(${formMeta.formName}, ${trans.call}) | success ${(trans.call == "createTicket")?' | ' + response.entryId:''}`);

                // gotta merge the field id linked indexes and merge 'em into the get fields list
                let fields = that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.query.fields.map((fName) => {
                    return(
                        /^\d+$/.test(fName)?fName:formMeta.formDef.nameIndex.hasOwnProperty(fName)?formMeta.formDef.nameIndex[fName].id:fName
                    )
                });
                ...
                new Promise((_t, _b) => {
                    ...
                    if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.postWriteSyncTransform instanceof Function){
                            that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.postWriteSyncTransform(formMeta, fetchTransaction, api, that).then((trn) => {
                                _t(trn);
                            }).catch((error) => {
...
writeCompletedTransaction(formMeta, dbRow, api, trans){
    ...
    return(new Promise((toot, boot) => {
        ...
        new Promise((__t, __b) => {
            // put the new row back into the target dataStore
            that.db.put({
                storeName: formMeta.storeName,
                object: dbRow
            }).then(() => {
                // if was create, delete the original guid-for-entryId row from the target dataStore
                if ((trans.call == "createTicket") && (trans.entryId.length > 15)){
                    that.db.deleteObject({
                        storeName: formMeta.storeName,
                        indexName: 'entryId',
                        key: trans.entryId
                    }).then(() => {
                ...
                // failed to write the updated row? this really shouldn't happen?
                writeFail = true;
                __b(`failed to write updated/created row`)
            })
...
handleTransmits(formMeta, api){
    ...
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`${fn}(${formMeta.formName}) | start`); }
        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'transmit', additionalDetail: 'fetching writeQueue', functionName: fn });
...
handlePrune(formMeta, syncOutput, api){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.db.getDescription(formMeta.storeName).then((d) => {
            rowCount = d[0].count;

            that.statusUpdate({
                message: `sync: ${formMeta.formName}`,
                detail: `pruning expired rows`,
                additionalDetail: `fetching writeQueue ...`,
            });
            ...
            that.db.getAll({
                ...
                if ((syncOutput.bulkSyncBypassed == true) && (
                    (! (
                        (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync instanceof Object) &&
                        that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.hasOwnProperty('disableSlowPrune') &&
                        (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.disableSlowPrune == true)
                    ))
                )){
                    ...
                    */
                    ...
                    that.db.openCursor({
                        ...
                        let queryChunks = [[]];
                        let queryPrefix = `(${that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.query.QBE}) AND (`;
                        checkQueue.forEach((entryId) => {
                            let currentChunk = queryChunks[(queryChunks.length -1)];
                            if (`${queryPrefix}'1'="${currentChunk.concat([entryId]).join(`" OR '1'="`)}")`.length < that._config.queryListByteLimit){
                        ...
                        Promise.all(queryChunks.map((chunk, idx) =>{return(new Promise((_t,_b) => {
                            ...
                            api.query({
                                schema: formMeta.formName,
                                fields: ['1'],
                                QBE: `${queryPrefix}'1'="${chunk.join(`" OR '1'="`)}")`
                            }).then((result) => {
...
/*
    handleWiteQueuePrune(formMeta, api)
...
handleWiteQueuePrune(formMeta,  api){
    let that = this;
    let fn = 'handleWiteQueuePrune';
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`${fn}(${formMeta.formName}) | start`); }
        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: 'get writeQueue', functionName: fn });
        that.db.getAll({
            storeName: 'writeQueue',
            indexName: 'statusSchema',
            query: IDBKeyRange.only(['transmitted', formMeta.formName])
        }).then((pruneQueueRaw) => {
            that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: `sort writeQueue (${pruneQueueRaw.length})`, functionName: fn });
            Promise.all(pruneQueueRaw.map((writeQueueRow) =>{return(new Promise((_t,_b)=>{

                new Promise((__t, __b) =>{
                    if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.writeQueueRowIsPrunable instanceof Function){
                        __t(that.config.dbConfig.storeDefinitions[formMeta.storeName]._sync.writeQueueRowIsPrunable(formMeta, writeQueueRow, api, that))
                    }else{
                    ...
                    if (deleteMe === true){
                        // delete em
                        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: `${writeQueueRow.entryId}`, functionName: fn });
                        that.db.deleteObject({
                            storeName: 'writeQueue',
                            indexName: 'queueID',
                ...
                that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: 'complete', functionName: fn });
                toot(true);
            }).catch((error) => {
...
export { noiceARSSyncWorkerThread };
