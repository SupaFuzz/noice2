...
set config(v){
    ...
    */
    if (
        ...
        this.statusUpdate({functionName: 'config attribute setter', message: 'parsing config', _status:'db-read' });
...
checkPendingUpgrade(){
    ...
    return(new Promise((toot, boot) => {
        that.statusUpdate({ message: 'mounting database', detail: 'check for pending upgrade', functionName: fn, _status:'net-read' });
...
mountDB(){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.statusUpdate({ message: 'mounting database', functionName: fn, _status:'db-read' });
        ...
        if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync instanceof Object){
            ...
            api.query(that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.metaQuery).then((metaQueryResult) => {
                if (that.config.dbConfig.storeDefinitions[formMeta.storeName]._bulkSync.hasNewDataCallback(metaQueryResult, formMeta, that)){
                    ...
                    */
                    that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'fetching max modified date from db', runAnimation:true, functionName: fn });
...
handleTransmits(formMeta, api){
    ...
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`${fn}(${formMeta.formName}) | start`); }
        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'transmit', additionalDetail: 'fetching writeQueue', functionName: fn, _status:'net-write' });
...
handleWiteQueuePrune(formMeta,  api){
    ...
    return(new Promise((toot, boot) => {
        if (that.debug){ that.log(`${fn}(${formMeta.formName}) | start`); }
        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: 'get writeQueue', functionName: fn, _status:'db-read' });
        that.db.getAll({
            storeName: 'writeQueue',
            indexName: 'statusSchema',
            query: IDBKeyRange.only(['transmitted', formMeta.formName])
        }).then((pruneQueueRaw) => {
            that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: `sort writeQueue (${pruneQueueRaw.length})`, functionName: fn, _status:'db-write' });
            Promise.all(pruneQueueRaw.map((writeQueueRow) =>{return(new Promise((_t,_b)=>{
                ...
                new Promise((__t, __b) =>{
                    ...
                    if (deleteMe === true){
                        // delete em
                        that.statusUpdate({ message: `sync: ${formMeta.formName}`, detail: 'prune writeQueue', additionalDetail: `${writeQueueRow.entryId}`, functionName: fn, _status:'db-write' });
                        that.db.deleteObject({
                            storeName: 'writeQueue',
                            indexName: 'queueID',
