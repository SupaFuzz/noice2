...
set config(v){
    ...
    if (v instanceof Object){
        ...
        /*
            ...
            thread.dbTagMappings[<dbTagName>] = <storeName>
        ...
        if (that._config.DBs instanceof Object){
            ...
            Object.keys(that._config.DBs).filter((dbTagName) => {return(
                ...
                Object.keys(that._config.DBs[dbTagName].storeDefinitions).filter((storeName) => {return(
                    ...
                    that.dbTagMappings[dbTagName] = storeName;
...
mountAll(){
    ...
    return(new Promise((toot, boot) => {
        if ((that.config instanceof Object) && (that.config.DBs instanceof Object)){
            ...
            that.log(`${fn}() | cannot find DBs in config?`);
            that.statusUpdate({
                messageNumber: 20,
                error: true,
                message: 'failed to mount databases (please contact administrator)',
                        ...
                        */
                        ...
                        api.getFormFields({ schema: formName }).then((formDef) => {
                            ...
                            if (
                                ...
                                (that.formDBMappings[formName] instanceof Object) &&
                                (that.config.DBs[that.formDBMappings[formName].dbTagName] instanceof Object) &&
                                (that.config.DBs[that.formDBMappings[formName].dbTagName].storeDefinitions instanceof Object) &&
                                (that.config.DBs[that.formDBMappings[formName].dbTagName].storeDefinitions[that.formDBMappings[formName].storeName] instanceof Object) &&
                                (that.config.DBs[that.formDBMappings[formName].dbTagName].storeDefinitions[that.formDBMappings[formName].storeName]._sync instanceof Object) &&
                                (that.config.DBs[that.formDBMappings[formName].dbTagName].storeDefinitions[that.formDBMappings[formName].storeName]._sync.formDefinitionTransform instanceof Object)
                            ){
                                formDef = that.config.DBs[that.formDBMappings[formName].dbTagName].storeDefinitions[that.formDBMappings[formName].storeName]._sync.formDefinitionTransform.formDefinitionTransform(formDef);
                            }
...
syncAllForms(){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.getAPI().then((api) => {
            ...
            /*
                NOTE 12/21/23 @ 1715
                we oughta build an iterator here rather than the dumb Promise.all
                only reason -- it makes the startup screen look like a mess flipping
                back and fourth between different process displays.
                not that it's a big deal but cleanliness matters
            */

            Promise.all(

                Object.keys(that.formDBMappings).filter((a) => {return(
            ...
            new Promise((_t,_b) => {
                ...
                that.handleTransmits(formMeta, api).then((entryIdIndex) => {
                    ...
                    */
                    ...
                    if (that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName]._sync.rowUpdateCallback instanceof Function){
                ...
                if ((syncOutput.bulkSyncBypassed == true) && (
                    ...
                    */
                    ...
                    that.DBs[formMeta.dbTagName].openCursor({
                        ...
                        let queryChunks = [[]];
                        let queryPrefix = `(${that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName]._sync.query.QBE}) AND (`;
