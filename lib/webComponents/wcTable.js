...
openPrefEditor(evt){
    ...
    if (that._elements.btnPrefs.dataset.open == 'true'){
        ...
        that.elements.btnExport.disabled = true;
    }else{
        that.closePanel();
        that.elements.btnExport.disabled = false;
    }
}
...
handleCellEdit(rowElement, cellElement){
    let that = this;
    return(new Promise((toot, boot) => {
        // too damn much for now, placeholder
        toot(true);
    }));
}
...
openPanel(DOMTree){
    ...
    if (DOMTree instanceof Element){
        that.prefEditorFrameThingy.append(that._elements.uiContainer);
