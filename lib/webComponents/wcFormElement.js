...
captureValue(evt){
    ...
    if (this.captureValueCallback instanceof Function){
        try{
            this.captureValueCallback(this.value);
        }catch(e){
...
setType(type, oldType){
    ...
    if (el instanceof Element){
        if (
            ...
            /*
                NOTE: only the 'change' and 'input' listeners really make sense for
                select elements. It is an intentional choice not to override it in here.

                this class is meant as a dumb front-end remote controlled by an intelligent
                backend. no logic should happen in here, which means if you change the type
                it's on you to set the appropriate capture_value_on as well :-)
            */

            // just call initialized callback again, that'll setup options and listeners for us
