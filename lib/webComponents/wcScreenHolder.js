...
addUI(screen, screenName){
    ...
    return(new Promise((toot, boot) => {
        new Promise((t) => {t(
            ...
            if (! screen.dataset.menu_order){ screen.dataset.menu_order = this.epochTimestamp(true); }
            if (! screen.dataset.menu_label){ screen.dataset.menu_label = screenName; }
...
getUIMenu(){
    ...
    return(new wcBasic({
        ...
        styleSheet: `
            ...
            /*
                5/27/28 @ 1111 NOTE: something like this would add an icon to a button
                but we need a way of defining the mask externally
                BRB off to write wcScreen.js
                ...
                button:after {
                    ...
                    width: 1em;
                    height: 1em;
                    background-color: var(--wc-screen-holder-menu-button-color);
                    margin-left: .5em;
                    mask: url('./gfx/buttons/burger.svg');
                    mask-size: contain;
            ...
            */
            ...
            initializedCallback: (slf) => {
                ...
                slf.generateMenu = () => {
                    let selectedUIName = ((that.currentUI instanceof Element) && (that.currentUI.dataset.name))?that.currentUI.dataset.name:null;
