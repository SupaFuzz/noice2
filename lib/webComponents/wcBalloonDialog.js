/*
    ...
    to-do list 5/9/24 @ 2256:
        ...
        * implement a full_screen flag. this is complicated esp for auto position but it has to happen
        * draggable
...
static classAttributeDefaults = {
    ...
    fullscreen: { observed: true, accessor: true, type: 'bool', value: true },
}
...
get defaultStyle(){return(`
    ...
    :host([show_decoration="true"][warning="true"]) div.dialog .body {
        border-color: var(--wc-balloon-dialog-body-warning-background-color, rgb(6, 133, 135));
    }
    :host([show_decoration="true"][alert="true"]) div.dialog .body {
        border-color: var(--wc-balloon-dialog-body-alert-background-color, rgb(230, 0, 161));
    }
    :host([show_decoration="true"]) div.dialog .body {
        ...
        border-color: var(--wc-balloon-dialog-body-background-color, rgb(191, 191, 24));
    }
    div.dialog .body {
        overflow: auto;
        max-width: 100%;
        box-shadow: var(--wc-balloon-dialog-box-shadow, .25em .25em .25em rgba(17, 47, 65, .5));
    }
    :host([show_decoration="false"]) div.dialogHeader {
        display: none;
    }
    :host([alert="true"]) div.dialogHeader {
        background-color: var(--wc-balloon-dialog-body-alert-background-color, rgb(230, 0, 161));
    }
    :host([warning="true"]) div.dialogHeader {
        background-color: var(--wc-balloon-dialog-body-warning-background-color, rgb(6, 133, 135));
    }
    div.dialogHeader {
        display: grid;
...
initializedCallback(){
    ...
    this.headerContent = this._headerContent;
...
setPositionRelativeTo(el, alignmentMode){
    if (el instanceof Element){
        ...
        /*
            full_screen: false
            meaning we are positioned within an element beneath document.body
            which means we need to emulate the output of getBoundingClientRect()
        ...
        */
        ...
        if (this.full_screen === false){
            ...
            const D = getComputedStyle(this.DOMElement);
            myD = {};
            ['left', 'width', 'bottom', 'height', 'top'].forEach((a) => { myD[a] = parseFloat(D.getPropertyValue(a)); });
            myD.x = myD.left;


            const T = getComputedStyle(el);
            targetD = {};
            ['left', 'width', 'bottom', 'height', 'top'].forEach((a) => { targetD[a] = parseFloat(T.getPropertyValue(a)); });
            targetD.x = targetD.left;
        }


        /*
            okaaaay ... I don't know why but this right here which basically
            just filters NaN values so we don't get CSS errors in the console ...
            it breaks relative positioning in full_screen:false mode
            but doesn't throw any CSS errors

            the version above just plays dumb and gets the CSS warnings in the console
            but it works for relative positioning and centered. <shruggin' lady emoji>
            meh. it works. why ask why?

        if (this.full_screen === false){

            const D = getComputedStyle(this.DOMElement);
            myD = {};
            ['x', 'left', 'width', 'bottom', 'height', 'top'].forEach((a) => { myD[a] = parseFloat(D.getPropertyValue(a)); });
            if (isNaN(myD.left) && (! isNaN(myD.x))){ myD.left = myD.x; }
            if (isNaN(myD.x) && (! isNaN(myD.left))){ myD.x = myD.left; }

            const T = getComputedStyle(el);
            targetD = {};
            ['x', 'left', 'width', 'bottom', 'height', 'top'].forEach((a) => { targetD[a] = parseFloat(T.getPropertyValue(a)); });
            if (isNaN(targetD.left) && (! isNaN(targetD.x))){ targetD.left = targetD.x; }
            if (isNaN(targetD.x) && (! isNaN(targetD.left))){ targetD.x = targetD.left; }

            // sigh ... hackery. there must be a better way than this
            [myD, targetD].forEach((d) => { Object.keys(d).filter((a) => {return(isNaN(d[a]))}).forEach((a) => {d[a] = 0; }); })
        ...
        */
        ...
        switch ((alignmentMode == 'auto')?this.arrow_position:alignmentMode){
            case 'topRight':
                ...
                if (targetD.width <= parseFloat(this.arrow_offset)){
                    this.x = `${((targetD.x + targetD.width) - myD.width + parseFloat(this.arrow_offset) + parseFloat(this.arrow_size)) - (targetD.width/2) }px`;
                }else{
                    this.x = `${(targetD.x + targetD.width) - myD.width}px`;
                }

                //HOOOLD UP -- why on earth does this work? with the Math.abs?
                this.y = `${Math.abs(targetD.bottom) + parseFloat(this.arrow_size)}px`;
                break;
            case 'topMiddle':
            ...
            case 'topLeft':
                ...
                if (targetD.width <= parseFloat(this.arrow_offset)){
                    this.x = `${targetD.x - parseFloat(this.arrow_offset) + (targetD.width/2)}px`;
                }else{
                    this.x = `${targetD.x}px`;
                }
                this.y = `${Math.abs(targetD.bottom) + parseFloat(this.arrow_size)}px`;
            ...
            case 'bottomRight':
                ...
                this.y = `${targetD.top - myD.height - parseFloat(this.arrow_size)}px`;
                break;
            case 'bottomMiddle':
            ...
            case 'rightTop':
                ...
                this.x = `${targetD.x - myD.width - parseFloat(this.arrow_size)}px`;
                this.y = `${(targetD.top + (targetD.height/2)) - parseFloat(this.veritical_arrow_offset) - parseFloat(this.arrow_size)}px`;
                break;
            case 'rightMiddle':
                // place the dialog's right edge along the target's left edge aligned center-to-center vertically
                this.x = `${targetD.x - myD.width - parseFloat(this.arrow_size)}px`;
                this.y = `${(targetD.top + (targetD.height/2)) - (myD.height/2) - parseFloat(this.arrow_size)}px`;
                break;
            case 'rightBottom':
                // place the dialog's right edge along the target's left edge centered at this.arrow_offset from bottom
                this.x = `${targetD.x - myD.width - parseFloat(this.arrow_size)}px`;
                this.y = `${(targetD.top + (targetD.height/2)) - myD.height + parseFloat(this.veritical_arrow_offset) - parseFloat(this.arrow_size)}px`;
                break;
            case 'leftTop':
                // place the dialog's left edge along the target's right edge centered at this.arrow_offset from top
                this.x = `${targetD.x + targetD.width + parseFloat(this.arrow_size)}px`;
                this.y = `${(targetD.top + (targetD.height/2)) - parseFloat(this.veritical_arrow_offset) - parseFloat(this.arrow_size)}px`;
                break;
            case 'leftMiddle':
