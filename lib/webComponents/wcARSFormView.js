...
/*
    holup -- changeFlag stuff

    how do we implement a change flag without a pointer to the arsRow?

    event: set_change_flag
    the change flag has changed value (details.oldValue, details.newValue)
    would this work for informing the backend of a UI change?

    ok I'm thinking of this wrong.
    this is the UI layer that has no concept of old and new value only
    current value.

    we do need a local copy of it though to enable logic like toggling
    the save button, and popping the "are you sure" and all that ...

    it's more like this

    UI field value change -> call event value_change: {field: name, value: yadda}
    arsRow listens to the event then responds by calling this.setChangeFlag()

    SOOO ...

    ok .. this.change_flag simply controls UI state.
    that is all. no callback because anything that could conceivably
    mutate the change_flag must be sent to the backend which makes the
    determination.
    ...
    this.change_flag (bool)
    is like a *shell* around the markup. If you set it false, it'll send the
    reset_change_flag event, and the owner can resets change flags in t
    yeah wait ///
