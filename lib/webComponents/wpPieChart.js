/*
    ...
    to-do:
        * addChart() - add mode:stack | overlay (but that's gonna require some trig so later LOL)
...
addChart(args){
    if (
        ...
        this._charts[args.name] = this.getPieChartPath(
            ...
            args.value,
            this.multiple_chart_mode?this.getMaxChartAngle():0
        );
        this._charts[args.name].dataset.name = args.name;
        this._charts[args.name].dataset.order = Object.keys(this._charts[args.name]).length + 1;
...
toggleChartMode(chartMode){
    if (this.initialized){
        ...
        Object.keys(this._charts).sort((a,b) => {return(
            ...
            if (chartMode == 'stack'){ sangle = isNaN(parseFloat(this._charts[name].dataset.angle))?0:parseFloat(this._charts[name].dataset.angle); }
        })
    }
}
