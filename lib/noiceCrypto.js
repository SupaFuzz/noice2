/*
    ...
    yolo!
*/
import { getGUID, noiceObjectCore, noiceCoreChildClass } from './noiceCore.js';
...
constructor(args, defaults, callback){
    ...
    // generate a salt
    this.salt = this.utf8ToUint8Array(getGUID());
}
...
encryptString(string, password){
    ...
    return(new Promise((toot, boot) => {
        ...
        that.encrypt(p, that.salt, d).then((c) => {
...
async decryptString(string, password){
    ...
    return(new Promise((toot, boot) => {
        ...
        this.decrypt(p, that.salt, s).then((b) => {
