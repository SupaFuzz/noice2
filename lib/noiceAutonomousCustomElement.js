    ...
    Object.keys(this.attributeDefaults).forEach((a) => {
        Object.defineProperty(this, a, {
            ...
            get: () => {
                return(
                    this.dataTypeMunger(
                        this.attributeDefaults[a].hasOwnProperty('type')?this.attributeDefaults[a].type:null,
                        (this.initialized && this.hasAttribute(a))?this.getAttribute(a):this.attributeDefaults[a].value
                    )
                );
            },
            set: (v) => {
                let type = this.attributeDefaults[a].hasOwnProperty('type')?this.attributeDefaults[a].type:null;
                if (this.initialized && this.hasAttribute(a)){
                    this.setAttribute(a, this.dataTypeMunger(type, v));
                }else{
                    let old = this.dataTypeMunger(type,this[a]);
                    this.attributeDefaults[a].value = this.dataTypeMunger(type,v);
...
dataTypeMunger(type, value){
    switch(type){
        ...
        /*
            LOH 4/10/24 @ 1637 -- COB
            I need a way to handle enum munging
            that's not easy unless I pass the entire enum value list in though
            dunno, maybe it doesn't belong here but ...
        */
