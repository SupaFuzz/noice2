...
dataTypeMunger(name, value){
    switch(this.attributeDefaults[name].hasOwnProperty('type')?this.attributeDefaults[name].type:null){
        ...
        case 'json':
            ...
            /*
                if input is string, we JSON.parse
                if input is an object, we JSON.stringify
                if input is an array, and we have key, build { key: []} and serialize/deserialize that
            */
            if (
            ...
            }else if (this.isNotNull(value)){
                let a = JSON.parse(value);
                if (
                    this.attributeDefaults[name].hasOwnProperty('key') &&
                    this.isNotNull(this.attributeDefaults[name].key) &&
                    a.hasOwnProperty(this.attributeDefaults[name].key)
                ){
...
attributeChangedCallback(name, oldValue, newValue){
    ...
    */
    if ( this.initialized && (this.attributeChangeHandlers instanceof Object) && (this.attributeChangeHandlers[name] instanceof Function) ){
        this.attributeChangeHandlers[name](name, this.dataTypeMunger(name, oldValue), this.dataTypeMunger(name, newValue), this);
    }
